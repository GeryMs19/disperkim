...
ol.control.Sidebar = function (settings) {
    ...
    this.element.classList.add('sidebar2-' + 'right');

    // Find sidebar > div.sidebar-content
    for (i = this.element.children.length - 1; i >= 0; i--) {
        child = this.element.children[i];
        if (child.tagName === 'DIV' &&
                child.classList.contains('sidebar2-content')) {
            this._container = child;
        }
    }

    // Find sidebar ul.sidebar-tabs > li, sidebar .sidebar-tabs > ul > li
    this._tabitems = this.element.querySelectorAll('ul.sidebar2-tabs > li, .sidebar2-tabs > ul > li');
    ...
    for (i = this._container.children.length - 1; i >= 0; i--) {
        ...
        if (child.tagName == 'DIV' &&
                child.classList.contains('sidebar2-pane')) {
            this._panes.push(child);

            var closeButtons = child.querySelectorAll('.sidebar2-close');
